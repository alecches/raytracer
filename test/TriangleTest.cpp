...
TEST(ParserTest, Gibberish) {
	ObjParser parser;
	parser.parse("../../../test/files/parseGibberish.txt");
	EXPECT_EQ(parser.ignoredLines(), 7);
}

TEST(ParserTest, Vertices) {
	ObjParser parser;
	parser.parse("../../../test/files/parseVertices.txt");
...
TEST(ParserTest, Triangles) {
	ObjParser parser;
	parser.parse("../../../test/files/parseTriangles.txt");
	std::vector<Tuple> vertices = parser.vertices();
...
TEST(ParserTest, Triangulate) {
	ObjParser parser;
	parser.parse("../../../test/files/parseTriangulate.txt");
...
TEST(ParserTest, TrianglesGroups) {
	ObjParser parser;
	parser.parse("../../../test/files/parseGroups.txt");
	std::vector<std::pair<std::string, Group>> grps = parser.groups();
...
TEST(ParserTest, VertexNormals) {
	ObjParser parser;
	parser.parse("../../../test/files/parseVN.txt");
...
TEST(ParserTest, VertexFaceNormals) {
	ObjParser parser;
	parser.parse("../../../test/files/parseFaceVN.txt");
	std::vector<Tuple> norms = parser.normals();
